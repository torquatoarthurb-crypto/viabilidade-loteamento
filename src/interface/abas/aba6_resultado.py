...
def _renderizar_cascata_dre(r: dict) -> None:
    ...
    _label_dre = "Resultado do Empreendimento" if _tem_parceiros_dre else "Resultado Bruto (Lucro Liquido)"
    ...
    if _tem_inv or _tem_ter:
        ...
        if _tem_inv:
            ...
            partes.append(
                f'<tr class="dre-row-deduction">'
                f'<td>&nbsp;&nbsp;&nbsp;(-) Participação Investidor ({pct_inv:.0f}% do resultado)</td>'
        ...
        if _tem_ter:
            ...
            partes.append(
                f'<tr class="dre-row-deduction">'
                f'<td>&nbsp;&nbsp;&nbsp;(-) Participação S. Terrenista ({pct_ter:.0f}% do resultado)</td>'
                f'<td>({formatar_brl(lucro_ter)})</td>'
                f'<td>({pct_vv(lucro_ter)})</td>'
                f'<td>({pct_vb(lucro_ter)})</td>'
                f'</tr>'
            )
            if pendente_ter > 1:
                partes.append(
